...
elseif RADIANCE_GLOBAL.plot_type == 1 % Spectrogram
    ...
    for k = 1:min(16,RADIANCE_GLOBAL.num_mics-ch_offset)
        ...
        imagesc(Tim+RADIANCE_GLOBAL.t(intv(k,1)), Freq/1e3, 10*log10(abs(Pwr)), ...
        ...
        if ~isnan(RADIANCE_GLOBAL.T_start( k+ch_offset, RADIANCE_GLOBAL.current_voc ))
            hold(RADIANCE_GLOBAL.handles.grid_axes(k), 'on');
            if RADIANCE_GLOBAL.T_start(k+ch_offset, RADIANCE_GLOBAL.current_voc) > min(Tim)+RADIANCE_GLOBAL.t(intv(k,1)) ...
               && RADIANCE_GLOBAL.T_start(k+ch_offset, RADIANCE_GLOBAL.current_voc) < max(Tim)+RADIANCE_GLOBAL.t(intv(k,1)) ...
               && RADIANCE_GLOBAL.F_start(k+ch_offset, RADIANCE_GLOBAL.current_voc) > min(Freq) ...
               && RADIANCE_GLOBAL.F_start(k+ch_offset, RADIANCE_GLOBAL.current_voc) < max(Freq)
                plot(RADIANCE_GLOBAL.handles.grid_axes(k), RADIANCE_GLOBAL.T_start(k+ch_offset, RADIANCE_GLOBAL.current_voc), ...
                     RADIANCE_GLOBAL.F_start(k+ch_offset, RADIANCE_GLOBAL.current_voc)/1e3, 'k.', 'linewidth', 1)
            end

            if RADIANCE_GLOBAL.T_stop( k+ch_offset, RADIANCE_GLOBAL.current_voc ) > min(Tim)+RADIANCE_GLOBAL.t(intv(k,1)) ...
               && RADIANCE_GLOBAL.T_stop( k+ch_offset, RADIANCE_GLOBAL.current_voc ) < max(Tim)+RADIANCE_GLOBAL.t(intv(k,1)) ...
               && RADIANCE_GLOBAL.F_stop( k+ch_offset, RADIANCE_GLOBAL.current_voc ) > min(Freq) ...
               && RADIANCE_GLOBAL.F_stop( k+ch_offset, RADIANCE_GLOBAL.current_voc ) < max(Freq)
                plot(RADIANCE_GLOBAL.handles.grid_axes(k), RADIANCE_GLOBAL.T_stop(k+ch_offset, RADIANCE_GLOBAL.current_voc), ...
                     RADIANCE_GLOBAL.F_stop(k+ch_offset, RADIANCE_GLOBAL.current_voc)/1e3, 'k.', 'linewidth', 1)
            end
            hold(RADIANCE_GLOBAL.handles.grid_axes(k), 'off');
        end
