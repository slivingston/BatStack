...
/* non-zero implies receipt of trigger signal, and the board
   should be recording posttrigger_len blocks of post-trigger
   samples. */
volatile unsigned int trigger_hit = 0; /* This is in most cases only cleared from within the DMA5 ISR. */
volatile unsigned int first_int_since_trigger = 1; /* This should only be cleared from within the DMA5 ISR.
                                                      It indicates whether the ISR is servicing the first interrupt
                                                      since trigger, which must be treated as a special case. */

/* [0] -- bits 7:0 correspond to Latch E at trigger time, and 15:14 (i.e. upper-most two bits) specify the sram_unit;
   [1] -- bits 15:4 correspond to Latch B at trigger time.
   
   Remaining bits not noted above are undefined (but may be used later, hence are "reserved").
   Note that this address might decrease by up to 3 words to account for triggers occuring in
   the midst of writing a 4-channel sample (this adjustment ensures alignment; otherwise,
   our numbering of recorded samples to channel numbers will be off, among other things.
*/
...
volatile unsigned int sram_unit = 0; /* Only 4 in current design; hence, sram_unit is in {0,1,2,3} */
unsigned int SRAMchipW[4] = {0x8002, 0x8004, 0x8008, 0x8010};
unsigned int SRAMchipR[4] = {0x8202, 0x8204, 0x8208, 0x8210};
volatile unsigned int num_dmablocks = 0;    /* Number of DMA transfer blocks to be written to each SRAM unit. */
